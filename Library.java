...
public class Library
{
  ...
  //private ArrayList<Podcast> 	podcasts;
	...
	public boolean download(AudioContent content)
	{
		...
		if (content.getType() == Song.TYPENAME){
			//forloop runs through the song array and checks if the song is already in the arraylist; produces error message if it is
  ...
  // Write a class SongLengthComparator that implements
	...
	{
		//checks to see if the index is valid, sets error message if not valid
   ...
   */
	...
	public boolean makePlaylist(String title)
	{
		...
		for(int i = 0; i < playlists.size(); i++){
			if(playlists.get(i).getTitle().equals(newPlaylist.getTitle())){
				//sets error message if it does exist
	...
	public boolean addContentToPlaylist(String type, int index, String playlistTitle)
	{
		...
		if(type.equalsIgnoreCase(Song.TYPENAME)){
			//checks if valid index, sets error message if not
			if (index < 1 || index > songs.size()){
				errorMsg = "Song Not Found";
				return false;
			}
  ...
  // Delete a song/audiobook/podcast from a playlist with the given title
	...
	public boolean delContentFromPlaylist(int index, String title)
	{
		...
		errorMsg = "Playlist Content Not Found";
		return false;
	}
